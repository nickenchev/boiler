...
void ImGuiVulkan::initialize(Renderer &renderer)
{
	...
	AssetId textureId = renderer.loadTexture(imageData, TextureType::RGBA_SRGB);
...
void ImGuiVulkan::render(Renderer &renderer, const FrameInfo &frameInfo)
{
	...
	for (int n = 0; n < drawData->CmdListsCount; n++)
	{
		...
		{
			std::vector<Vertex> vertices(cmd_list->VtxBuffer.size());
			for (ImDrawVert v : cmd_list->VtxBuffer)
			{
				vec4 colour((v.col & 0xff000000) >> 24, // R
							(v.col & 0x00ff0000) >> 16, // G
							(v.col & 0x0000ff00) >> 8,  // B
							v.col & 0x000000ff); // A
				vertices.push_back(Vertex(vec3(v.pos.x, v.pos.y, 0), vec2(v.uv.x, v.uv.y), colour));
			}

			std::vector<uint32_t> indices(cmd_list->IdxBuffer.size());
		...
		for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
		{
			...
			else
			{
				...
				groups[0].primitives.push_back(MaterialGroup::PrimitiveInstance(primitiveId, matrixId));
			}
		}
